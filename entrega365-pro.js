...
(function(){
  ...
  async function status(){const mail=email();if(!mail)throw new Error('Entre usando sua conta Google para verificar o PRO.');const r=await fetch('/api/pro-status?email='+encodeURIComponent(mail)+(sub()?'&subscription_id='+encodeURIComponent(sub()):''),{cache:'no-store',headers:{Accept:'application/json'}}),d=await readJson(r);if(!r.ok)throw new Error(d.message||d.error||'Não foi possível consultar a assinatura.');if(d.subscription_id)localStorage.setItem(SUB_KEY,String(d.subscription_id));const previous=getPlan();const next=d.active?'active':'free';setPlan(next);localStorage.setItem(SYNC_KEY,String(Date.now()));if(previous!==next)window.dispatchEvent(new CustomEvent('e365-pro-updated',{detail:{active:d.active,status:d.status||''}}));return d}
