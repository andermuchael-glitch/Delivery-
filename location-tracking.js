(function(){
  ...
  function open(){
    ...
    async function toggleTracking(){const b=card.querySelector('#e365LocStart');b.disabled=true;try{if(trackingOn()){await window.entrega365Location.stop();card.querySelector('#e365LocStatus').textContent='⚪ Rastreamento desativado.'}else{await window.entrega365Location.start();const p=localPoint();if(p)renderLocal(p,'🟠 Rastreamento ativo; sincronizando...');await flush();await refresh()}}catch(e){card.querySelector('#e365LocStatus').textContent='🔴 Não foi possível ativar. Verifique a permissão de localização do navegador/Android.'}finally{b.disabled=false;updateControl()}}
