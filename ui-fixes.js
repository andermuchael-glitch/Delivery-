(()=>{
const STYLE=`
#app .day-layout-fixed{}
#app main.day-ui main{}
/* Campos maiores e mais confortáveis no celular */
...
#app .day-ui .add{min-height:50px;font-size:15px;margin:0 0 12px}
...
@media(max-width:600px){#app .day-ui .entry{grid-template-columns:minmax(0,1fr) minmax(0,1fr) 36px 30px}#app .day-ui .title{font-size:18px}#app .day-ui .add{position:relative}}
`;
...
function apply(){css();let main=document.querySelector('#app main');if(!main)return;let isDay=!!main.querySelector('#date')&&!main.querySelector('#expdate')&&!main.querySelector('#last');if(!isDay)return;main.classList.add('day-ui');
 ...
 // Fechamento: leva os totais e a arrancada para um cartão no topo, antes das comandas.
 ...
 // A arrancada fica junto do fechamento.
 ...
 // Nova comanda fica imediatamente abaixo do título, antes dos campos.
 let add=cmdCard.querySelector(':scope > .add');let title=cmdCard.querySelector(':scope > .title');if(add&&title&&title.nextElementSibling!==add)title.insertAdjacentElement('afterend',add);
