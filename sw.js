...
self.addEventListener("fetch",event=>{
  ...
  if(req.mode==="navigate"){
    event.respondWith(fetch(req).then(injectAuth).then(res=>{const copy=res.clone();caches.open(CACHE).then(c=>c.put("./index.html",copy)).catch(()=>{});return res;}).catch(()=>caches.match("./index.html")));return;
  }
