...
(async function startAuthRecovery(){try{if(auth.currentUser&&getSessionUid()===auth.currentUser.uid){openApp(auth.currentUser,{persist:true});return;}if(openSavedSession())return;setLoading();startupTimer=setTimeout(()=>{if(auth.currentUser)openApp(auth.currentUser,{persist:true});else if(!openSavedSession()){recoveryFinished=true;loginInProgress=false;sessionStorage.removeItem(LOGIN_PENDING);showLogin();}},2500);}catch(e){console.error("Firebase startup:",e);if(!openSavedSession()){recoveryFinished=true;showLogin();}}})();
