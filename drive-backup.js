...
export function initDriveBackup(auth){
  ...
  async function tok(interactive=false){if(token&&Date.now()<exp-60000)return token;if(loadToken())return token;if(!interactive)throw Error("drive_authorization_required");const u=auth.currentUser;if(!u)throw Error("login_required");const p=new GoogleAuthProvider();p.addScope("https://www.googleapis.com/auth/drive.file");p.addScope("https://www.googleapis.com/auth/drive.appdata");p.setCustomParameters({login_hint:mail(),prompt:"select_account",include_granted_scopes:"true"});const z=await reauthenticateWithPopup(u,p),q=GoogleAuthProvider.credentialFromResult(z);if(!q?.accessToken)throw Error("drive_token");putToken(q.accessToken,Date.now()+3500000);return token}
  async function api(url,opt={}){const t=await tok(false);const res=await fetch(url,{...opt,cache:"no-store",headers:{Authorization:"Bearer "+t,...(opt.headers||{})}});if(!res.ok)throw Error("drive_"+res.status);return res.status===204?null:res.json()}
  ...
  function keys(){const o={};for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&k!==TK&&k!==TE&&k!==stateKey()&&includedKey(k))o[k]=localStorage.getItem(k)}return o}
