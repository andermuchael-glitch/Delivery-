const CACHE="entrega365-v47";
const ASSETS=["./","./index.html?v=47","./logo-moto.svg?v=47","./manifest.json?v=47"];
const LOGO_FIX=`<style id="entrega365-logo-fix">
.logo{width:46px!important;height:46px!important;border-radius:10px!important;background-color:#111!important;background-image:url("logo-moto.svg?v=47")!important;background-position:center!important;background-size:contain!important;background-repeat:no-repeat!important;border:1px solid #ffbd19!important;}
.biglogo{width:min(340px,88vw)!important;height:min(340px,88vw)!important;margin:0 auto 14px!important;border-radius:0!important;background-color:transparent!important;background-image:url("logo-moto.svg?v=47")!important;background-position:center!important;background-size:contain!important;background-repeat:no-repeat!important;}
</style>`;
...
self.addEventListener("fetch",event=>{if(event.request.method!=="GET")return;const url=new URL(event.request.url);if(event.request.mode==="navigate"||url.pathname.endsWith("/index.html")){event.respondWith(logoFixedResponse(event.request));return;}if(url.pathname.endsWith("/sw.js")){event.respondWith(fetch(event.request,{cache:"no-store"}));return;}event.respondWith(caches.match(event.request).then(response=>response||fetch(event.request)));});