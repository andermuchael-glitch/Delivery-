...
export async function ensureSchema(sql) {
  ...
  schemaPromise = (async () => {
    await sql`CREATE TABLE IF NOT EXISTS community_posts (id BIGSERIAL PRIMARY KEY, author_uid TEXT NOT NULL DEFAULT '', author_name TEXT NOT NULL DEFAULT 'Entregador', author_email TEXT NOT NULL DEFAULT '', text TEXT NOT NULL DEFAULT '', type TEXT NOT NULL DEFAULT 'text', url TEXT NOT NULL DEFAULT '', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`;
    ...
    await sql`CREATE TABLE IF NOT EXISTS community_comments (id BIGSERIAL PRIMARY KEY, post_id BIGINT NOT NULL REFERENCES community_posts(id) ON DELETE CASCADE, author_uid TEXT NOT NULL DEFAULT '', author_name TEXT NOT NULL DEFAULT 'Entregador', text TEXT NOT NULL DEFAULT '', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`;
    ...
    await sql`CREATE TABLE IF NOT EXISTS community_likes (post_id BIGINT NOT NULL REFERENCES community_posts(id) ON DELETE CASCADE, author_uid TEXT NOT NULL DEFAULT '', user_uid TEXT NOT NULL DEFAULT '', created_at TIMESTAMPTZ NOT NULL DEFAULT NOW())`;
    ...
    await sql`UPDATE community_likes SET author_uid=CASE WHEN NULLIF(author_uid,'') IS NOT NULL THEN author_uid ELSE user_uid END,user_uid=CASE WHEN NULLIF(user_uid,'') IS NOT NULL THEN user_uid ELSE author_uid END`;
    await sql`DELETE FROM community_likes WHERE NULLIF(author_uid,'') IS NULL`;
    await sql`DELETE FROM community_likes a USING community_likes b WHERE a.ctid<b.ctid AND a.post_id=b.post_id AND a.author_uid=b.author_uid`;
    await sql`DO $$ DECLARE r RECORD; BEGIN FOR r IN SELECT c.conname FROM pg_catalog.pg_constraint c JOIN pg_catalog.pg_class t ON t.oid=c.conrelid JOIN pg_catalog.pg_namespace n ON n.oid=t.relnamespace WHERE n.nspname='public' AND t.relname='community_likes' AND c.contype='p' LOOP EXECUTE format('ALTER TABLE public.community_likes DROP CONSTRAINT %I',r.conname); END LOOP; END $$`;
    await sql`CREATE UNIQUE INDEX IF NOT EXISTS community_likes_post_author_uid_idx ON community_likes(post_id,author_uid)`;
    ...
    await sql`CREATE TABLE IF NOT EXISTS marketplace_settings (id SMALLINT PRIMARY KEY DEFAULT 1 CHECK (id=1), affiliate_url TEXT NOT NULL DEFAULT '', items_json TEXT NOT NULL DEFAULT '[]', updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_by_uid TEXT NOT NULL DEFAULT '')`;
    await sql`ALTER TABLE marketplace_settings ADD COLUMN IF NOT EXISTS affiliate_url TEXT NOT NULL DEFAULT ''`;
    await sql`ALTER TABLE marketplace_settings ADD COLUMN IF NOT EXISTS items_json TEXT NOT NULL DEFAULT '[]'`;
