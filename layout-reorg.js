/* Entrega365 — reorganização do cabeçalho e menu Mais */
(function(){
  'use strict';
  ...
  function openMarketplace(){
    ...
    let n=0;
    const t=setInterval(()=>{
      const x=document.querySelector('.cm-tab[data-v="marketplace"]');
      if(x){clearInterval(t);x.click();}
  ...
  function showMore(){
    ...
    m.innerHTML=`<div class="e365-more-layout-card"><div class="e365-more-layout-handle"></div><div class="e365-more-layout-title">Mais recursos</div><div class="e365-more-layout-sub">Ferramentas e opções do Entrega365</div><div class="e365-more-layout-grid">
      ...
      <button data-e365-more="language">🌐<br>Idioma</button>
      <button data-e365-more="exit">🚪<br>Sair</button>
    </div></div>`;
    document.body.appendChild(m);
    m.addEventListener('click',e=>{
      if(e.target===m){m.remove();return;}
      const b=e.target.closest('[data-e365-more]');if(!b)return;
      const x=b.dataset.e365More;m.remove();
      ...
      else if(x==='language')clickLanguage();
      else if(x==='exit')clickExit();
    });
  }
  function build(){
    styles();
    ...
    if(a){
      ...
      const plus=bs.find(b=>/^\s*\+\s*$/.test((b.textContent||'')))||bs[bs.length-1];
      if(plus)plus.style.display='none';
      const lang=bs.find(b=>/🌐|idioma|language/i.test((b.textContent||'')));
      const exit=bs.find(b=>/🚪|sair|logout|exit/i.test((b.textContent||'')));
      if(lang)lang.style.display='none';
      if(exit)exit.style.display='none';
    }
    let nav=document.querySelector('.e365-social-nav');
    if(!nav){
      nav=document.createElement('nav');nav.className='e365-social-nav';
      nav.innerHTML='<button type="button" data-e365-social="community">👥 Comunidade</button><button type="button" data-e365-social="marketplace">🛍️ Marketplace</button>';
      const h=document.querySelector('header');
      if(h)h.insertAdjacentElement('afterend',nav); else document.body.prepend(nav);
      nav.addEventListener('click',e=>{const b=e.target.closest('[data-e365-social]');if(!b)return;if(b.dataset.e365Social==='community')openCommunity();else openMarketplace();});
    }
  }
