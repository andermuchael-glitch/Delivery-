...
(function(){
  ...
  function screen(){
    ...
    return `<div class="e365prohero">
      ...
      ${active?'<button class="primary" id="e365procheck">ATUALIZAR STATUS</button>':'<button class="primary" id="e365probuy">ASSINAR POR R$ 9,90</button>'}
      ${!active&&getSub()?'<button class="e365prosecondary" id="e365procheck">JÁ ASSINEI — VERIFICAR</button>':''}
      ${!active&&!getSub()?'<button class="e365prosecondary" id="e365prolegacy">PAGAR PELO LINK DO MERCADO PAGO</button>':''}
      <div class="e365pronote">A assinatura é recorrente. O sistema consulta o status diretamente no Mercado Pago e só considera PRO quando a assinatura estiver autorizada.</div>
    </div>
  ...
  async function startSubscription(){
    ...
    try{
      ...
      console.warn(e);
      alert(e.message||'Não foi possível iniciar a assinatura.');
      if(b){b.disabled=false;b.textContent='ASSINAR POR R$ 9,90'}
    }
  }
