(function(){
  ...
  function open(){
    ...
    async function toggleTracking(){const b=card.querySelector('#e365LocStart');b.disabled=true;try{if(trackingOn()){await window.entrega365Location.stop();card.querySelector('#e365LocStatus').textContent='⚪ Rastreamento desativado.'}else{await window.entrega365Location.start();const p=localPoint();sessionLast=null;sessionMeters=0;if(p)renderLocal(p,'🟢 Rastreamento ativo — sincronizando localização...');await flush();await refresh()}}catch(e){card.querySelector('#e365LocStatus').textContent=e?.message==='background_location_required'?'🟡 Abra as configurações do Entrega365 e selecione localização “Permitir o tempo todo”. Depois volte ao app e ative novamente.':'🔴 Não foi possível ativar. Verifique a permissão de localização do navegador/Android.'}finally{b.disabled=false;updateControl()}}
