(function(){
  ...
  async function load(){const t=await auth();const r=await fetch(API+'?limit=300',{headers:{Authorization:'Bearer '+t},cache:'no-store'});if(!r.ok)throw new Error('HTTP '+r.status);return r.json()}
  function open(){
    ...
    function onLocation(e){renderLocal(e.detail,'🟠 Localização capturada; sincronizando PostgreSQL...');flush().then(refresh).catch(()=>{})}
    async function toggleTracking(){const b=card.querySelector('#e365LocStart');b.disabled=true;try{const on=trackingOn()?!(await window.entrega365Location.stop(),false):await window.entrega365Location.start();updateControl();if(on){const p=localPoint();if(p)renderLocal(p,'🟠 Rastreamento ativo; sincronizando...');await flush();await refresh()}else{card.querySelector('#e365LocStatus').textContent='⚪ Rastreamento desativado.'}}catch(e){card.querySelector('#e365LocStatus').textContent='🔴 Não foi possível ativar. Verifique a permissão de localização do navegador/Android.'}finally{b.disabled=false;updateControl()}}
    ...
    async function refresh(){try{await flush();const local=localPoint();const data=await load();latest=data.latest||local||null;if(local&&(!data.latest||Number(local.timestamp)>Number(data.latest.timestamp||0)))renderLocal(local,trackingOn()?'🟠 Rastreamento ativo; último ponto local aguardando confirmação...':'🟠 Último ponto local');else card.querySelector('#e365LocStatus').textContent=latest?'🟢 Último ponto recebido do PostgreSQL':'🟡 Nenhum ponto sincronizado ainda';card.querySelector('#e365Lat').textContent=latest?coord(latest.latitude):'—';card.querySelector('#e365Lng').textContent=latest?coord(latest.longitude):'—';card.querySelector('#e365Acc').textContent=latest&&latest.accuracy!=null?Math.round(latest.accuracy)+' m':'—';card.querySelector('#e365Time').textContent=latest?time(latest.timestamp):'—';card.querySelector('#e365LocList').innerHTML=(data.points||[]).slice(-15).reverse().map(p=>'<div><span>'+time(p.timestamp)+'</span><strong>'+coord(p.latitude)+', '+coord(p.longitude)+'</strong><small>±'+(p.accuracy==null?'—':Math.round(p.accuracy)+' m')+'</small></div>').join('')||'<em>Nenhum registro ainda.</em>'}catch(e){card.querySelector('#e365LocStatus').textContent='Não foi possível carregar os dados. Verifique a conexão e o login.'}}
