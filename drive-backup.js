...
export function initDriveBackup(auth){
  ...
  async function manualBackup(){
    ...
    try{
      const u=auth.currentUser;if(!u)throw Error("login_required");
      await tok(true);
      const ok=await save();
      if(ok){setState({manualSavedAt:Date.now(),manualSaveOk:true});return true}
      return false;
    }catch(e){console.warn("Backup manual:",e);setState({manualSaveOk:false,manualSaveError:String(e?.code||e?.message||e),manualSaveStatus:e?.status||0});return false}
  }
